...
def plot_heuristics(problem, nom_instance, max_configs=30, step=5, seed=42):
    """
    Genere un graphique montrant l'evolution de la duree de vie en fonction
    du nombre de configurations pour les trois heuristiques.
    """
    try:
        import matplotlib.pyplot as plt
    except ImportError:
        print("Erreur : matplotlib n'est pas installe. Lancez 'pip install matplotlib'.")
        return

    n_values = list(range(step, max_configs + 1, step))

    heuristics = ["greedy", "hef", "aleatoire"]
    couleurs  = {"greedy": "blue", "hef": "green", "aleatoire": "red"}
    marqueurs = {"greedy": "-o",   "hef": "-s",    "aleatoire": "-^"}
    ...
    for h in heuristics:
        plt.plot(n_values, results[h], marqueurs[h], color=couleurs[h], label=h.capitalize())

    plt.title(f"Influence du nombre de configurations ({nom_instance})")
    plt.xlabel("Nombre de configurations initiales")
    plt.ylabel("Duree de vie optimale")
    plt.legend()
